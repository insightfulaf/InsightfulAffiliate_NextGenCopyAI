...
def read_text(path: Path) -> str:
    """
    Read a text file robustly. Prefer UTF-8; if that fails, try common
    fallbacks; finally replace undecodable bytes so the agent can proceed.

    Raises:
        UnicodeDecodeError: If the file appears to be binary (contains NUL bytes).
    """
    TRIES = [
        ("utf-8", "strict"),
        ("utf-8", "replace"),
        ("cp1252", "strict"),   # Windows-1252
        ("mac_roman", "strict") # old Mac encoding
    ]
    for enc, err in TRIES:
        try:
            return path.read_text(encoding=enc, errors=err)
        except UnicodeDecodeError:
            continue
    data = path.read_bytes()
    if b"\x00" in data:
        raise UnicodeDecodeError("utf-8", data, 0, 1, "appears binary")
